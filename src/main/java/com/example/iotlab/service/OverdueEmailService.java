...
public class OverdueEmailService {
    ...
    public void sendOverdueWarningEmails() {
        ...
        for (Borrow borrow : allBorrows) {
            ...
            // Only process active (not returned) borrows
            ...
            // Check if overdue (dueDate has passed)
            if (!dueDate.isBefore(today)) {
                continue;
            }

            // Skip if we already sent a warning for this exact borrow
            if (emailLogRepository.existsByBorrowIdAndEmailType(
                    borrow.getId(), "OVERDUE_WARNING")) {
                continue;
            }

            // Look up student's registered email by name
            String studentEmail = resolveStudentEmail(borrow.getStudentName());

            // Case 1: No registered email found for this student
            if (studentEmail == null) {
                emailLogRepository.save(new EmailNotificationLog(
                        ...
                        "OVERDUE_WARNING",
                        "NO_EMAIL",
                        "Student '" + borrow.getStudentName() + "' has no registered email"
                ));
                emailsSent++;
                continue;
            }

            // Case 2: Try sending the email
            String failureReason = sendFineWarningEmail(
                    studentEmail,
            ...
            boolean sent = (failureReason == null);
            ...
            emailLogRepository.save(new EmailNotificationLog(
                    ...
                    "OVERDUE_WARNING",
                    sent ? "SENT" : "FAILED",
                    sent ? null : failureReason
            ));
            emailsSent++;
        }

        if (emailsSent > 0) {
            System.out.println("[SmartLab] Overdue warning emails sent: " + emailsSent);
        }
    }

    // ── Send a single fine-warning email ─────────────────────────────────
    ...
    public String sendFineWarningEmail(String toEmail, String studentName,
        ...
        try {
            ...
            System.err.println("[SmartLab] Failed to send email to " + toEmail + ": " + reason);
            return reason; // non-null = failure reason
        }
    }

    // ── Look up email from student name ───────────────────────────────────
    ...
    private String buildEmailHtml(String studentName, String equipmentName,
        ...
        return "<!DOCTYPE html>"
                ...
                + "  .info-box h2 { color: #b91c1c; font-size: 16px; margin: 0 0 14px; display: flex; align-items: center; gap: 8px; }"
                ...
                + "  .fine-box .amount { font-size: 48px; font-weight: 900; margin: 8px 0; letter-spacing: -1px; }"
                ...
                + "  .steps { margin: 24px 0; }"
                + "  .step-table { width: 100%; border-collapse: collapse; margin-bottom: 12px; }"
                + "  .step-num { width: 32px; height: 32px; background: #0d9488; color: #ffffff; border-radius: 50%; font-size: 13px; font-weight: 700; text-align: center; vertical-align: middle; padding: 0; }"
                + "  .step-text { font-size: 14px; color: #374151; line-height: 1.6; padding-left: 12px; vertical-align: middle; }"
                ...
                + // Header
                "  <div class='header'>"
                + "    <h1>🔬 Smart Lab Equipment System</h1>"
                + "    <p>Overdue Equipment — Fine Warning Notice</p>"
                + "  </div>"
                + // Body
                "  <div class='body'>"
                + "    <p class='greeting'>Dear " + studentName + ",</p>"
                + "    <p style='color:#374151;font-size:14px;line-height:1.7;'>We noticed that the lab equipment borrowed under your account is <strong>overdue</strong>. "
                + "    Please return it immediately to avoid a financial penalty.</p>"
                + // Borrow details
                "    <div class='info-box'>"
                + "      <h2>⚠️ Overdue Borrow Details</h2>"
                ...
                + // Fine box
                "    <div class='fine-box'>"
                + "      <div class='label'>Fine if not returned within 2 days</div>"
                + "      <div class='amount'>₹500</div>"
                + "      <div style='font-size:13px;opacity:0.85;'>per overdue borrow record</div>"
                + "    </div>"
                + // Deadline
                "    <div class='deadline-box'>"
                + "      <div class='title'>⏰ Return Deadline to Avoid Fine</div>"
                ...
                + "      <table class='step-table'><tr><td class='step-num'>1</td><td class='step-text'>Log in to the Smart Lab System at <strong>localhost:3000</strong></td></tr></table>"
                + "      <table class='step-table'><tr><td class='step-num'>2</td><td class='step-text'>Go to <strong>Return Equipment</strong> from your Student Dashboard</td></tr></table>"
                + "      <table class='step-table'><tr><td class='step-num'>3</td><td class='step-text'>Enter your Borrow ID <span class='borrow-id'>#" + borrowId + "</span> and the quantity you are returning</td></tr></table>"
                + "      <table class='step-table'><tr><td class='step-num'>4</td><td class='step-text'>Submit the return form — your record will be updated immediately</td></tr></table>"
                + "    </div>"
                + "    <p style='font-size:13px;color:#6b7280;margin-top:24px;line-height:1.6;'>If you believe this is an error or have already returned the equipment, "
                + "    please contact the lab administrator immediately.</p>"
                + "  </div>"
                + // Footer
                "  <div class='footer'>"
                + "    <p>This is an automated message from Smart Lab Equipment System.</p>"
